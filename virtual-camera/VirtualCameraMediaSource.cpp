...
class MediaSource final : public IMFMediaSourceEx, public IMFGetService {
    std::atomic<ULONG> refs_{1};
    IMFMediaEventQueue* events_=nullptr;
...
extern "C" __declspec(dllexport) HRESULT STDAPICALLTYPE DllRegisterServer() {
    ...
    LONG rc=RegCreateKeyExW(HKEY_LOCAL_MACHINE,key.c_str(),0,nullptr,0,KEY_SET_VALUE,nullptr,&h,nullptr); if(rc!=ERROR_SUCCESS)return HRESULT_FROM_WIN32(rc);
...
extern "C" __declspec(dllexport) HRESULT STDAPICALLTYPE DllUnregisterServer() {
    wchar_t clsid[64]{}; StringFromGUID2(CLSID_4KRustCameraVirtualSource,clsid,64);
    std::wstring key=L"Software\\Classes\\CLSID\\"+std::wstring(clsid); LONG rc=RegDeleteTreeW(HKEY_LOCAL_MACHINE,key.c_str());
