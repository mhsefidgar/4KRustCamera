...
extern "C" __declspec(dllexport) HRESULT STDMETHODCALLTYPE DllGetClassObject(REFCLSID clsid,REFIID riid,void**ppv){
...
extern "C" __declspec(dllexport) HRESULT STDMETHODCALLTYPE DllRegisterServer() {
    ...
    wchar_t clsid[64]{}; StringFromGUID2(CLSID_4KRustCameraVirtualSource,clsid,64);
    std::wstring key=L"Software\\Classes\\CLSID\\"+std::wstring(clsid)+L"\\InprocServer32"; HKEY h=nullptr;
    LONG rc=RegCreateKeyExW(HKEY_CURRENT_USER,key.c_str(),0,nullptr,0,KEY_SET_VALUE,nullptr,&h,nullptr);
...
extern "C" __declspec(dllexport) HRESULT STDMETHODCALLTYPE DllUnregisterServer() {
    ...
    LONG rc=RegDeleteTreeW(HKEY_CURRENT_USER,key.c_str()); return rc==ERROR_FILE_NOT_FOUND?S_OK:HRESULT_FROM_WIN32(rc);
}
